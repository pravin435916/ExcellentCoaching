...
const ReviewCard = ({ name, comment, stars }) => {
  const starIcons = [];
...
const Reviews = () => {
  return (
    <div className="flex flex-col items-center gap-4 justify-center p-4 ">
      <h1 className='text-4xl sm:text-5xl font-bold text-center'>Hear from Our Satisfied Families</h1>
      ...
      <ReviewCard name="John Doe" stars={5} comment="My daughter's grades have improved significantly since\n starting tutoring with Acme. The personalized attention\n and flexible scheduling have been a game-changer." />
      <ReviewCard name="Jane Smith" stars={4} comment="Good experience overall. Great service, highly recommended!" />
      <ReviewCard name="Michael Johnson" stars={3} comment="Could be better, but not bad. Great service, highly recommended!" />
      </div>
    </div>
  );
};
